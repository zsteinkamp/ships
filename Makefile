...
devup: ## Build and run in development mode
	cd ships_dev && docker compose build && docker compose up -d --force-recreate && docker compose logs -f

devlogs: ## Tail development container logs
	cd ships_dev && docker compose logs -f

devdown: ## Stop development containers
	cd ships_dev && docker compose down
